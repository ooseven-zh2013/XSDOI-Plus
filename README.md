...
| 板块美化 | 玻璃外观三选一：**亚克力（毛玻璃）/ 液态玻璃 / 仅透明化**，亮/暗色自适应，可调透明度；亚克力为背景模糊，液态玻璃**不做模糊**（背景保持清晰）而靠边缘高光 / 伪折射营造玻璃质感，可另开**实验性真折射**（canvas 采样背景做边缘几何扭曲，需页面有背景图才明显、性能开销较大）。题目列表表格（vxe-table）、竞赛排行榜搜索框与榜单设置按钮、代码速打页卡片（ct-lib/ct-card/紫色 ct-hero）同样应用玻璃背景 | popup「板块美化」 |
