...
| 板块美化 | 玻璃外观三选一：**亚克力（毛玻璃）/ 液态玻璃 / 仅透明化**，亮/暗色自适应，可调透明度。亚克力 = 背景模糊（`backdrop-filter: blur(20px) saturate(180%)`）；液态玻璃**不做模糊**（背景保持清晰），靠「边缘折射 + 边缘高光 + 浮动阴影」营造玻璃质感：`::before` 折射层承载边缘白带（伪折射）与 SVG `feDisplacementMap` 真折射滤镜（实验性开关，边缘挤压变形、中间不变形），`::after` 高光层做顶部亮弧 + 内侧亮线 + 底部外投影。题目列表表格（vxe-table）、竞赛排行榜搜索框与榜单设置按钮、代码速打页卡片（ct-lib/ct-card/紫色 ct-hero）同样应用玻璃背景 | popup「板块美化」 |
