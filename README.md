...
| 网页桌宠 | 可拖动的圆球小宠物，贴底走动/蹦跳，边缘吸附，可放入自定义图片并在裁剪器中拖动/缩放圆形选区（重开面板选区正确还原）；显隐与裁剪点「保存配置」后立即应用，由 popup 控制显隐；全局注入，所有页面均显示；拖到空中松手后遵循抛物线惯性落地；与鼠标尾迹联动——碰到圆点尾迹时反弹，碰到带状尾迹时根据速度方向反弹或滑行；双击宠物弹出悬浮聊天窗口（API 地址、模型名、API Key 在 popup「网页桌宠」面板中统一设置，自动填入聊天窗口，无需手动输入；居中卡片+毛玻璃遮罩，点击遮罩空白处或点 × 关闭） | popup「网页桌宠」 |
